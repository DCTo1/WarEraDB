...
exact API lifetime rankings (userDamages / userBounty / userWealth) — so fetched users
...
class Filler:
    ...
    NOTE: there is no third pool for account_created_at (migration_23). The
    API stopped serving getUserLite dates.createdAt, so no fetch can ever
    fill it — upsert_stmts derives the column from the user's ObjectID
    instead, and fillers.CreatedAtBackfillFiller (whose candidate query could
    never shrink, since every fetch left the column NULL) was deleted
    2026-08-16.

    KNOWN INEFFICIENCY (measured 2026-08-16, not yet fixed): _refill falls
    back to the head of the queue when this shard's slice is empty, so once a
    pool is shorter than shards x REFILL every filler-carrying cycle step
    picks the SAME users — 148 getUserLite calls for 41 distinct users in 5
    minutes (~3.6x). Harmless (idempotent upserts) but it is most of the
    endpoint's current traffic; a shard_owns() filter on the fallback rows
    would collapse it.
...
def upsert_stmts(fetched: dict) -> list[str]:
    ...
    account_created_at (migration_23) is derived from the user's ObjectID
    (leading 4 bytes = Unix seconds), NOT from the API: getUserLite's
    dates.createdAt no longer exists (verified 2026-08-16 on 10 users; the
    column was NULL for all 116,483 rows). Plain overwrite is fine — the
    value never changes. For accounts that predate the 2025-05-01 game
    restart this is the restart's document date rather than the original
    signup, which is the right value for every use it has (transaction
    ranges, activity windows) but not a claim about how long someone has
    played."""
    stmts = []
    for h, d in fetched.items():
        ...
        for col, v in (
            ...
            ("account_created_at", f"to_timestamp({int(h[:8], 16)})"),
